...
#include <cassert>
#include <cstring>
#pragma warning(disable: 4828) // arning C4828: The file contains a character starting at offset 0xa9 (and 0x3bc) that is illegal in the current source character set (codepage 65001).
// cant get the warning to go away, but it does not seem to affect the code
#include "FreeImage.h"
...
{
	//swap value if they are inverted
	if (x1 >= x2) { int temp = x2; x2 = x1; x1 = temp; }
	if (y1 >= y2) { int temp = y2; y2 = y1; y1 = temp; }

	// Early exit if the rectangle is fully out of bounds
	if (x1 >= m_Width || x2 < 0 || y1 >= m_Height || y2 < 0) return;

	//out of bound check
	x1 = x1 <= 0 ? 0 : x1;
	x2 = (x2 >= m_Width) ? m_Width -1: x2;
	y1 = y1 <= 0 ? 0 : y1;
	y2 = (y2 >= m_Height) ? m_Height -1: y2;
		...
		if ((srcwidth > 0) && (srcheight > 0))
		{
			...
			for ( int y = 0; y < srcheight; y++ )
			{
				for (int x = 0; x < srcwidth; x++) {
					if (src[x] & 0xff'ff'ff) dst[x] = src[x];
				}
